...
for i = 1:8 % we will keep s, the shrinking factor, within [.1,.8] to avoid the trivial cases
    ...
    for j = 1:k
        ...
        % calculate the pairwise distance between all points in cluster j and the shrunken representative points
        % and find all points that are less than average std away from the shrunken representative points
        sumf = 0;
        for jcount = 1:length(Xj)
            d = pdist2(Xj(jcount,:),v);
            f = d < stdev_a;
            sumf = sumf + sum(f,1);
            clear f
        end
        
        card = sumf/size(Xj,1); % compute cardinality of each shrunken representative point
